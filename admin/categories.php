    ...
    try {
        if ($action === 'add' || $action === 'edit') {
            ...
            while (true) {
                ...
                if (!$stmt->fetch()) break; // Slug is unique
            ...
            if ($action === 'add') {
                ...
                $_SESSION['success_message'] = "Category '" . e($name) . "' added successfully.";
                log_admin_activity("Created category: " . e($name), 'category', $last_id);
            } else { // 'edit'
                $id = (int)$_POST['id'];
                ...
                log_admin_activity("Updated category: " . e($name), 'category', $id);
            }
            ...
            if ($id > 0) {
                ...
                if ($stmt->fetchColumn() > 0) {
                    throw new Exception("Cannot delete. It has sub-categories. Please move or delete them first.");
                }
                ...
                log_admin_activity("Deleted category ID: " . $id, 'category', $id);
            }
        }
    } catch (Exception $e) {
...
// Fetch all categories for display on the page
...
// Fetch only top-level categories for the dropdowns in modals
$parent_categories_stmt = $pdo->query("SELECT id, name FROM categories WHERE parent_id IS NULL ORDER BY name ASC");
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-hover" width="100%">
                <thead>
                    <tr><th>Category Name</th><th>Parent</th><th>Status</th><th class="text-center">Featured</th><th class="text-center">Actions</th></tr>
                </thead>
                <tbody>
                    <?php if (empty($categories)): ?>
                        <tr><td colspan="5" class="text-center">No categories found. Add one to get started!</td></tr>
                    <?php else: ?>
                        <?php foreach ($categories as $cat): ?>
                            <tr>
                                <td><strong><?php echo e($cat['name']); ?></strong><br><small class="text-muted">/<?php echo e($cat['slug']); ?></small></td>
                                <td><?php echo e($cat['parent_name'] ?? '<em>—</em>'); ?></td>
                                <td><?php echo $cat['is_published'] ? '<span class="badge bg-success">Published</span>' : '<span class="badge bg-warning text-dark">Draft</span>'; ?></td>
                                <td class="text-center"><?php echo $cat['is_featured'] ? '<i class="fas fa-star text-warning"></i>' : '<i class="far fa-star text-muted"></i>'; ?></td>
                                <td class="text-center">
                                    ...
                                    <button class="btn btn-sm btn-danger delete-btn" title="Delete" data-bs-toggle="modal" data-bs-target="#deleteConfirmationModal" data-id="<?php echo e($cat['id']); ?>"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
<div class="modal fade" id="editCategoryModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="categories.php" method="POST" id="editCategoryForm">
...
<div class="modal fade" id="deleteConfirmationModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">Are you sure? This action cannot be undone. Products in this category will be unlinked. Sub-categories must be removed first.</div>
            <div class="modal-footer">
                <form id="confirmDeleteForm" action="categories.php" method="POST">
                    <input type="hidden" name="action" value="delete"><input type="hidden" name="id" id="deleteId"><input type="hidden" name="csrf_token" value="<?php echo e($csrf_token); ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editModal) {
        editModal.addEventListener('show.bs.modal', function(event) {
            ...
            // Disable the category from being its own parent in the dropdown
            const parentSelect = document.getElementById('editParentId');
            for (let i = 0; i < parentSelect.options.length; i++) {
                if (parentSelect.options[i].value == catId) {
