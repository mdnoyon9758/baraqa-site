...
try {
    ...
    // Fetch total clicks for today
    $today_clicks = $pdo->query("SELECT COUNT(id) FROM affiliate_clicks WHERE DATE(click_time) = CURDATE()")->fetchColumn();
    ...
    for ($i = 6; $i >= 0; $i--) {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM affiliate_clicks WHERE DATE(click_time) = ?");
        $stmt->execute([$date]);
        $clicks_last_7_days['labels'][] = date('M d', strtotime($date));
        $clicks_last_7_days['data'][] = (int)$stmt->fetchColumn();
    }
    $chart_data_json = json_encode($clicks_last_7_days['data']);
    $chart_labels_json = json_encode($clicks_last_7_days['labels']);

} catch (PDOException $e) {
    echo '<div class="alert alert-danger">Could not fetch dashboard data. Error: ' . e($e->getMessage()) . '</div>';
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <div class="btn-toolbar mb-2 mb-md-0">
        <a href="products.php?action=add" class="btn btn-sm btn-primary"><i class="fas fa-plus me-2"></i>Add Product</a>
        <a href="settings.php" class="btn btn-sm btn-outline-secondary ms-2"><i class="fas fa-cog"></i> Site Settings</a>
    </div>
</div>
...
<div class="row">
    <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-left-primary shadow h-100 py-2">
            <div class="card-body"><div class="row no-gutters align-items-center"><div class="col mr-2"><div class="text-xs font-weight-bold text-primary text-uppercase mb-1">Live Products</div><div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo e($total_products); ?></div></div><div class="col-auto"><i class="fas fa-box-open fa-2x text-gray-300"></i></div></div><a href="products.php" class="stretched-link"></a></div>
        </div>
    </div>
    <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-left-info shadow h-100 py-2">
            <div class="card-body"><div class="row no-gutters align-items-center"><div class="col mr-2"><div class="text-xs font-weight-bold text-info text-uppercase mb-1">Total Brands</div><div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo e($total_brands); ?></div></div><div class="col-auto"><i class="fas fa-bookmark fa-2x text-gray-300"></i></div></div><a href="manage_brands.php" class="stretched-link"></a></div>
        </div>
    </div>
    <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-left-success shadow h-100 py-2">
            <div class="card-body"><div class="row no-gutters align-items-center"><div class="col mr-2"><div class="text-xs font-weight-bold text-success text-uppercase mb-1">Total Categories</div><div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo e($total_categories); ?></div></div><div class="col-auto"><i class="fas fa-tags fa-2x text-gray-300"></i></div></div><a href="categories.php" class="stretched-link"></a></div>
        </div>
    </div>
    <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-left-warning shadow h-100 py-2">
            <div class="card-body"><div class="row no-gutters align-items-center"><div class="col mr-2"><div class="text-xs font-weight-bold text-warning text-uppercase mb-1">Clicks (Today)</div><div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo e($today_clicks); ?></div></div><div class="col-auto"><i class="fas fa-mouse-pointer fa-2x text-gray-300"></i></div></div><a href="clicked_leads.php" class="stretched-link"></a></div>
        </div>
    </div>
</div>
